...
typedef struct
{
  ...
  SpeednPosFdbk_Handle_t *AUXSPD;      /*!< The Auxiliary speed sensor  */
...
/* Select the sensor as primary sensor if the angle error is below the authorized margin */
...
/**
  * @brief Returns the auxiliary speed sensor
  * @param pHandle: handler of the current instance of the SpeednTorqCtrl component.
  * @retval SpeednPosFdbk_Handle_t auxiliary speed sensor
  *
  * - Called as soon as component parameters are required by MC FW.
  */
static inline SpeednPosFdbk_Handle_t *STC_GetAuxSensor(SpeednTorqCtrl_Handle_t *pHandle)
{
#ifdef NULL_PTR_CHECK_SPD_TRQ_CTL
  return ((MC_NULL ==  pHandle) ? MC_NULL : pHandle->AUXSPD);
#else
  return (pHandle->AUXSPD);
#endif
}
/**
  * @}
